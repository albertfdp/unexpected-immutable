...
const MapType = {
  ...
  inspect: function (collection, depth, output, inspect) {
    ...
    const maxLineLength = output.preferredWith - (depth === Infinity ? 0 : depth) * 2 - 2
    let width = 0
    const compact = inspectedItems.length > 5 || inspectedItems.every(item => {
    ...
    keys.forEach((key, idx) => {
      output
        ...
        .block(function () {
          ...
          valueOutput.amend(type.delimiter(output.clone(), idx, actualKeys.length))
          if (!isInlineDiff) {
